...
import mathactthree from './mathactthree';
...
function MathMaze() {
  ...
  const getNextQuestion = (prevCorrect, emotion, history) => {
    ...
    const candidates = mathactthree.filter(q => q.level === level && !history.includes(q.question));
  ...
  useEffect(() => {
    if (completed) {
      ...
      const markActivityComplete = async () => {
        ...
        if (!progressData['Math'].includes('Maze')) {
          progressData['Math'].push('Maze');
    ...
    return (
      <div className="maze-wrapper">
        ...
        <div className="completion-box">
          ...
          <button onClick={handleRestart}>Restart Maze</button>
        </div>
      </div>
    );
  }
  ...
  if (!currentQ) {
    return <div className="maze-wrapper"><h2>No questions available.</h2></div>;
  }
